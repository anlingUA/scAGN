...
scMap_prediction_inter <- function(refdf, querydf, reflabel,  querylabel, folder_to_save, dataset_name)
{
    ...
    sce <- SingleCellExperiment(list(normcounts = mycount.list[[1]]), colData = data.frame(cell_type1 = mylabel.list[[1]]$type) )
    logcounts(sce) <- log2(normcounts(sce) + 1)
    # use gene names as feature symbols
    rowData(sce)$feature_symbol <- rownames(sce)
    sce <- selectFeatures(sce, suppress_plot = TRUE)

    sce_test <- SingleCellExperiment(list(normcounts = mycount.list[[2]] ), colData = data.frame(cell_type1 = mylabel.list[[2]]$type  ))
    ...
    True_Labels_scmapcluster <- list(mylabel.list[[2]]$type)
    Pred_Labels_scmapcluster <- list(scmapCluster_results$combined_labs)
    prediction_match = unlist(True_Labels_scmapcluster) == unlist(Pred_Labels_scmapcluster)
    final_accuracy <- (sum(prediction_match)/length(prediction_match))
    fprintf("[scMap-Cluster] Dataset = %s, Final Prediction is = %f\n", dataset_name, final_accuracy, file = sprintf("%s/scMap_predict.txt",folder_to_save ),append=TRUE)
    ...
    True_Labels_scmapcell <- list(mylabel.list[[2]]$type)
    Pred_Labels_scmapcell <- list(scmapCell_clusters$combined_labs)

    prediction_match = unlist(True_Labels_scmapcell) == unlist(Pred_Labels_scmapcell)
    final_accuracy <- (sum(prediction_match)/length(prediction_match))
    fprintf("[scMap-Cell] Dataset = %s, Final Prediction is = %f\n\n", dataset_name, final_accuracy, file = sprintf("%s/scMap_predict.txt",folder_to_save ),append=TRUE)
...
scMap_prediction_intra <- function(data, labels, folder_to_save, dataset_name)
{
    refdf <- data.frame(matrix(ncol = length(colnames(data)), nrow = 0))
    reflabel <- data.frame(matrix(ncol = 1, nrow = 0))
    querydf <- data.frame(matrix(ncol = length(colnames(data)), nrow = 0))
    ...
    write.csv(mylabel.list[[2]], file = sprintf("%s/%s/true_labels_scMAP-cluster.csv", folder, dataset_name) )
    write.csv(as.data.frame(Pred_Labels_scmapcluster), file = sprintf("%s/%s/predicted_labels_scMAP-cluster.csv", folder, dataset_name) )
    
    prediction_match = unlist(True_Labels_scmapcluster) == unlist(Pred_Labels_scmapcluster)
    final_accuracy <- (sum(prediction_match)/length(prediction_match))
    fprintf("[scMap-Cluster] Dataset = %s, Final Prediction is = %f\n", dataset_name, final_accuracy, file = sprintf("%s/scMap_predict.txt",folder_to_save ),append=TRUE)
    fprintf("%s,%f\n", dataset_name, final_accuracy, file = sprintf("%s/scMAP-cluster_metric.txt",folder_to_save ),append=TRUE)
    fprintf("%s,%f\n", dataset_name, total_time_taken, file = sprintf("%s/scMP-cluster_time_taken.txt",folder_to_save ),append=TRUE)
    ...
    write.csv(mylabel.list[[2]], file = sprintf("%s/%s/true_labels_scMAP-cell.csv", folder, dataset_name) )
    write.csv(as.data.frame(Pred_Labels_scmapcell), file = sprintf("%s/%s/predicted_labels_scMAP-cell.csv", folder, dataset_name) )
    
    prediction_match = unlist(True_Labels_scmapcell) == unlist(Pred_Labels_scmapcell)
    final_accuracy <- (sum(prediction_match)/length(prediction_match))
    fprintf("[scMap-Cell] Dataset = %s, Final Prediction is = %f\n\n", dataset_name, final_accuracy, file = sprintf("%s/scMap_predict.txt",folder_to_save ),append=TRUE)
    fprintf("%s,%f\n", dataset_name, final_accuracy, file = sprintf("%s/scMAP-cell_metric.txt",folder_to_save ),append=TRUE)
    fprintf("%s,%f\n", dataset_name, total_time_taken, file = sprintf("%s/scMAP-cell_time_taken.txt",folder_to_save ),append=TRUE)
    print("Done!")
...
if(dataset == "GSE165092") {
    ...
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = "GSE165092")
    ...
    print(head(labels))
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = "GSE108989")
    ...
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    scMap_prediction_intra(tdataGSE72056, labels, folder_to_save = "./", dataset_name = "GSE72056")
    ...
    rownames(labels) <- rows
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    scMap_prediction_intra(tdataGSE85241, labels, folder_to_save = "./", dataset_name = "GSE85241")
    ...
    print(head(labels))
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = "GSE98638")
    ...
    print(head(labels))
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = "GSE99254")
    ...
    rownames(labels) <- rows
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    scMap_prediction_intra(tdataGSE118389, labels, folder_to_save = "./", dataset_name = "GSE118389")
    ...
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    scMap_prediction_intra(dataGSE83139, labels, folder_to_save = "./", dataset_name = "GSE83139")
    ...
    scMap_prediction_inter(dataGSE85241, dataGSE81076, labelsGSE85241,  labelsGSE81076, folder_to_save = "./", dataset_name = "GSE85241_GSE81076")
    ...
    dataGSE85241 <- dataGSE85241[,common_genes]
    dataGSE81076 <- dataGSE81076[,common_genes]
    scMap_prediction_inter(dataGSE81076, dataGSE85241, labelsGSE81076, labelsGSE85241, folder_to_save = "./", dataset_name = "GSE81076_GSE85241")
    ...
    scMap_prediction_inter(dataGSE83139, dataGSE84133, labelsGSE83139, labelsGSE84133, folder_to_save = "./", dataset_name = "GSE83139_GSE84133")
    ...
    dataGSE83139 <- dataGSE83139[,common_genes]
    dataGSE84133 <- dataGSE84133[,common_genes]
    scMap_prediction_inter(dataGSE84133, dataGSE83139, labelsGSE84133, labelsGSE83139, folder_to_save = "./", dataset_name = "GSE84133_GSE83139")
    ...
    scMap_prediction_inter(dataGSE84133_human, dataGSE84133_mouse, labelsGSE84133_human, labelsGSE84133_mouse, folder_to_save = "./", dataset_name = "GSE84133_human_mouse")
    ...
    fprintf("Shape of dataset is [%d x %d]\n", dim(dataGSE84133_mouse)[1],dim(dataGSE84133_mouse)[2] )
    fprintf("Shape of labels df is [%d x %d]\n", dim(labelsGSE84133_mouse)[1],dim(labelsGSE84133_mouse)[2] )
    scMap_prediction_inter(dataGSE84133_mouse, dataGSE84133_human, labelsGSE84133_mouse, labelsGSE84133_human, folder_to_save = "./", dataset_name = "GSE84133_mouse_human")
    ...
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    scMap_prediction_intra(dataGSE124952, labels, folder_to_save = "./", dataset_name = "GSE124952")
    ...
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    scMap_prediction_intra(dataGSE103892, labels, folder_to_save = "./", dataset_name = "GSE103892")
} else if(dataset == 'xin') {
    ...
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = "Xin")
} else if(dataset == 'TM') {
    main_directory <- sprintf("%s/%s", folder, 'ADClust_datasets')
    fprintf("%s\n",main_directory)
    ...
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    stopifnot(identical(rownames(data), rownames(labels)))
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = "TM")
    ...
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    stopifnot(identical(rownames(data), rownames(labels)))
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = "Tasic")
    ...
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    stopifnot(identical(rownames(data), rownames(labels)))
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = "Segerstolpe")
    ...
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    stopifnot(identical(rownames(data), rownames(labels)))
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = "Romanov")
    ...
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    stopifnot(identical(rownames(data), rownames(labels)))
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = "PBMC_68k")
    ...
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    stopifnot(identical(rownames(data), rownames(labels)))
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = "Mouse_retina")
    ...
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    stopifnot(identical(rownames(data), rownames(labels)))
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = "Klein")
    ...
    fprintf("Shape of labels df is [%d x %d]\n", dim(labels)[1],dim(labels)[2] )
    stopifnot(identical(rownames(data), rownames(labels)))
    scMap_prediction_intra(data, labels, folder_to_save = "./", dataset_name = dataset)   
