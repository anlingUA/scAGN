...
library(pracma)
options(future.globals.maxSize = 8000 * 4096^2)
...
#' @examples: load count.list and label.list from folder "example_data"
#' save_processed_data(count.list,label.list)
save_processed_data <- function(count.list,label.list, folder, Rgraph=TRUE,check_unknown=FALSE){
    fprintf("Folder to save graph is %s\n", folder)
    
    count.list <- pre_process(count_list=count.list,label_list=label.list)
    #' save counts data to certain path: 'input'
    dir.create(folder); 
    write.csv(t(count.list[[1]]),file=sprintf('%s/Data1.csv', folder),quote=F,row.names=T)
    write.csv(t(count.list[[2]]),file=sprintf('%s/Data2.csv', folder),quote=F,row.names=T)
    ...
    if (Rgraph){
        ...
        write.csv(graphs[[1]],file=sprintf('%s/inter_graph.csv', folder),quote=F,row.names=T)
        write.csv(graphs[[2]],file=sprintf('%s/intra_graph.csv', folder),quote=F,row.names=T)
        write.csv(new.lab1,file=sprintf('%s/Label1.csv',folder),quote=F,row.names=F)
        write.csv(new.lab2,file=sprintf('%s/Label2.csv',folder),quote=F,row.names=F)        
    } else {
